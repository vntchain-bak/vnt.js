...
describe('lib/vnt/batch', function () {
    describe('execute', function () {
        it('should execute batch request', function (done) {
            ...
            provider.injectValidation(function (payload) {
                ...
                assert.equal(first.method, 'core_getBalance');
                assert.deepEqual(first.params, ['0x0000000000000000000000000000000000000000', 'latest']);
                assert.equal(second.method, 'core_getBalance');
            ...
            batch.add(vnt.core.getBalance.request('0x0000000000000000000000000000000000000005', 'latest', callback2));
            batch.execute();
        });
        ...
        it('should execute batch request for async properties', function (done) {
            ...
            provider.injectValidation(function (payload) {
                ...
                assert.equal(first.method, 'core_accounts');
            ...
            var batch = vnt.createBatch();
            batch.add(vnt.core.getAccounts.request(callback));
            batch.add(vnt.net.getPeerCount.request(callback2));
            batch.execute();
        });
        ...
        it('should execute batch request with contract', function (done) {
            ...
            provider.injectValidation(function (payload) {
                ...
                assert.equal(first.method, 'core_getBalance');
                assert.deepEqual(first.params, ['0x0000000000000000000000000000000000000000', 'latest']);
                assert.equal(second.method, 'core_call');
                assert.deepEqual(second.params, [{
