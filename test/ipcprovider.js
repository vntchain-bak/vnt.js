...
var IpcProvider = SandboxedModule.require('../lib/vnt/ipcprovider', {
    requires: {
...
describe('lib/vnt/ipcprovider', function () {
    describe('send', function () {
        it('should send basic request', function () {
            var provider = new IpcProvider('', net);
            var result = provider.send({id: 1, method: 'core_test'});
    ...
    describe('sendAsync', function () {
        it('should send basic async request', function (done) {
            var provider = new IpcProvider('', net);
            provider.sendAsync({id: 1, method: 'core_test'}, function (err, result) {
                assert.isObject(result);
                done();
            });
        });
