...
describe('vnt.core.contract', function() {
    it('should create simple contract with one method from abi with explicit type name', function () {
        ...
        // when
        var vnt = new Vnt();
        var myCon = vnt.core.contract(description).at(address);
    ...
    it('should create contract with nondefault constructor', function (done) {
        ...
        provider.injectValidation(function (payload) {
            if (steps === 1) {
                ...
                assert.equal(payload.jsonrpc, '2.0');
                assert.equal(payload.method, 'core_newBlockFilter');
                steps++;
            }
        });

        vnt.core.contract(description).new(2, {from: address, data: code}, function(e, myCon){
                done();
                // vnt.stopWatching();
        });
    });
